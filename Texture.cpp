...
{
	if (gRender->gpuResManager->textureMap.count(fileName) > 0)
	{
		texture = gRender->gpuResManager->textureMap[fileName];
	}
	else
	{
		...
		gRender->gpuResManager->textureMap[fileName] = texture;
	}
}